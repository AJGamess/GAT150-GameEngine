...
AudioComponent::AudioComponent(const AudioComponent& other)
{
    ...
    {
        m_audioSource = std::make_unique<AudioSource>(*other.m_audioSource);
    }
}
...
void AudioComponent::Update(float dt)
{
    if (playOnStart)
    {
        playOnStart = false; // Ensure it doesn't play again on the next update
        Play();
    }
}
